...
local function create(className, props, children)
    ...
    if children then for _,c in ipairs(children) do if c then c.Parent = inst end end end
    return inst
end
local function makeRounded(obj, r) create("UICorner", { CornerRadius = UDim.new(0, r), Parent = obj }) end
local function keycodeToString(kc) return (tostring(kc):match("([^%.]+)$")) or tostring(kc) end
local function tweenSizeSafe(obj, newSize, ed, es, dur)
...
-- Accent pub/sub
local AccentSubs = {}
local function accent_sub(fn) table.insert(AccentSubs, fn); return fn end
local function accent_pub(c) for _,fn in ipairs(AccentSubs) do pcall(fn,c) end end
...
local DefaultTheme = {
    ...
    stroke=Color3.fromRGB(70,70,84), buttonBase=Color3.fromRGB(36,36,44), buttonHover=Color3.fromRGB(46,46,56),
    bad=Color3.fromRGB(240,80,95)
}

-- Optional FPS cap helper (best-effort)
local function applyFpsCap(v)
    v = math.clamp(tonumber(v) or 60, 10, 240)
    local tried = {}
    local function try(fn, label)
        if type(fn)=="function" then table.insert(tried,label); local ok=pcall(fn,v); if ok then return true end end
        return false
    end
    local names = { "setfpscap","set_fps_cap","fpscap","setfps" }
    for _,n in ipairs(names) do if try(rawget(_G,n), n) then return true end end
    local envs = { getgenv and getgenv() or nil, _G, getrenv and getrenv() or nil, _ENV }
    for _,e in ipairs(envs) do if type(e)=="table" then for _,n in ipairs(names) do if try(rawget(e,n),"env."..n) then return true end end end end
    if syn and type(syn.set_fps_cap)=="function" and try(syn.set_fps_cap,"syn.set_fps_cap") then return true end
    if type(setfflag)=="function" then local ok=pcall(function() setfflag("DFIntTaskSchedulerTargetFps", tostring(v)) end); if ok then return true end end
    warn("[EclipseUI] No FPS cap function found. Tried: "..table.concat(tried,", "))
    return false
end
...
function EclipseUI:CreateWindow(cfg)
    ...
    local initialSize = (responsive and UIS.TouchEnabled) and mobileSize or defaultDesktopSize
    ...
    makeRounded(root,16)
    create("UIStroke",{Parent=root, Color=theme.panel2, Transparency=0.25})
    ...
    create("TextLabel", {
        ...
        Text = tostring(cfg.Title or "Eclipse UI"),
    ...
    local banner = create("Frame", {
        ...
        Position = UDim2.new(0,0,0,-bannerH+1),
        ZIndex = 5,
        Parent = root
    })
    ...
    create("TextLabel",{
        ...
        Text="This script & menu was made by doug, this is not a copy of any script and it made by me only",
        TextColor3=theme.text,Font=Enum.Font.GothamMedium,TextSize=13,TextWrapped=true,
        TextXAlignment=Enum.TextXAlignment.Center,TextYAlignment=Enum.TextYAlignment.Center,Parent=banner,ZIndex=6
    })

    -- Toggle key + hint
    local uiToggleKey = cfg.ToggleKey or Enum.KeyCode.RightShift

    local hint = create("Frame", {
        Name="ReopenHint", Visible=false, BackgroundColor3=theme.panel2, BorderSizePixel=0,
        AnchorPoint=Vector2.new(0.5,0), Position=UDim2.new(0.5,0,0,6), ZIndex=300, Parent=gui
    })
    makeRounded(hint,8)
    local hintStroke = create("UIStroke",{Parent=hint, Color=theme.accent, Transparency=0.2})
    ...
    local hintText = create("TextLabel",{
        BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),Text="",TextColor3=theme.text,
        Font=Enum.Font.Gotham,TextSize=13,TextXAlignment=Enum.TextXAlignment.Center,TextYAlignment=Enum.TextYAlignment.Center,
        Parent=hint,ZIndex=301
    })
    local function updateHintText()
        ...
        hint.Size = UDim2.fromOffset(math.clamp(sz.X + hintPad.PaddingLeft.Offset + hintPad.PaddingRight.Offset, 120, 420),
                                     math.max(28, sz.Y + hintPad.PaddingTop.Offset + hintPad.PaddingBottom.Offset))
    end
    updateHintText()
    ...
    do
        ...
        UIS.InputChanged:Connect(function(input)
            ...
            root.Position = UDim2.new(startPos.X.Scale, math.floor(startPos.X.Offset + delta.X + 0.5),
    ...
    -- Tabs bar + body (scroll)
    local tabsBar = create("Frame", { Size = UDim2.new(0,160,1,-titleH), Position = UDim2.new(0,0,0,titleH), BackgroundColor3 = theme.panel, BorderSizePixel = 0, Parent = root })
    ...
    local bodyScroll = create("ScrollingFrame", {
        Size = UDim2.new(1,-160,1,-titleH),
        Position = UDim2.new(0,160,0,titleH),
        BackgroundColor3 = theme.panel,
        BorderSizePixel = 0,
    ...
    local body = create("Frame", { BackgroundTransparency = 1, Size = UDim2.fromScale(1,1), Parent = bodyScroll })
    ...
    local tipText = create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,Parent=tooltip})
    local function showTooltip(str)
        tipText.Text = str
    ...
    local function attachTooltip(target, text)
        ...
        target.MouseEnter:Connect(function()
            token = token + 1; local me = token
            task.delay(0.25, function()
                if me == token and target:IsDescendantOf(gui) then showTooltip(text) end
            end)
        end)
        target.MouseLeave:Connect(function() token = token + 1; hideTooltip() end)
    end
    ...
    local function updateDock()
        ...
        elseif mode=="window-right" then
            px,py = snap(rx + rw + gap), snap(ry + ((dockCfg.align=="content") and (titleH+gap+offsetY) or offsetY))
        else
            px,py = snap(rx - gap), snap(ry + ((dockCfg.align=="content") and (titleH+gap+offsetY) or offsetY))
        end
        NotifDock.Position = UDim2.fromOffset(px,py)
        NotifDock.Size = UDim2.fromOffset(dockCfg.width or 260, rh)
    end
    RunService.RenderStepped:Connect(updateDock); task.defer(updateDock)

    -- Control row helper
    function EclipseUI:_makeControlRow(parent, height, fullWidth)
        local padLeft, padRight = 0, 0
        for _,ch in ipairs(parent:GetChildren()) do
            if ch:IsA("UIPadding") then padLeft = ch.PaddingLeft.Offset; padRight = ch.PaddingRight.Offset; break end
        end
        local holder = Instance.new("Frame")
        holder.Size = UDim2.new(1,0,0,height); holder.BackgroundTransparency = 1; holder.Parent = parent
        local mode = (fullWidth == true or fullWidth == "edge") and "edge" or "normal"
        local bleedL, bleedR = 0, 0
        if mode == "edge" then bleedL = padLeft; bleedR = padRight end
        local row = Instance.new("Frame")
        row.Size = UDim2.new(1, bleedL+bleedR, 1, 0)
        row.Position = UDim2.new(0, -bleedL, 0, 0)
        row.BackgroundColor3 = theme.buttonBase; row.BorderSizePixel = 0; row.Parent = holder
        makeRounded(row,8); create("UIStroke",{Parent=row,Color=theme.stroke,Transparency=0.2})
        local pad = Instance.new("UIPadding"); pad.PaddingLeft=UDim.new(0,12); pad.PaddingRight=UDim.new(0,12); pad.Parent=row
        return holder, row
    end

    -- Tabs API
    function EclipseUI:AddTab(name)
        name = tostring(name)
        ...
        local function AddSection(which, title)
            ...
            function section:AddButton(cfgb)
                cfgb = cfgb or {}
                local holder,row = EclipseUI:_makeControlRow(group,34,cfgb.fullWidth)
                local btn = create("TextButton",{BackgroundTransparency=1,Size=UDim2.fromScale(1,1),Text=tostring(cfgb.text or "Button"),Font=Enum.Font.GothamBold,TextSize=14,TextColor3=theme.text,Parent=row})
                btn.AutoButtonColor=false
                attachTooltip(btn, cfgb.tooltip)
                btn.MouseButton1Click:Connect(function() if type(cfgb.callback)=="function" then task.spawn(cfgb.callback) end end)
                return btn
            end

            function section:AddToggle(cfgt)
                cfgt = cfgt or {}
                local state = cfgt.default or false
                local holder,row = EclipseUI:_makeControlRow(group,34,cfgt.fullWidth)
                local lbl = create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-56,1,0),Text=tostring(cfgt.text or "Toggle"),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
                ...
                attachTooltip(row, cfgt.tooltip)
                local function set(v, silent)
                    ...
                    if not silent and type(cfgt.callback)=="function" then task.spawn(cfgt.callback, state) end
                end
                ...
                accent_sub(function(c) if state then knob.BackgroundColor3 = c end end)
                return { Set=function(_,v) set(v) end, Get=function() return state end }
            end

            -- SLIDER (touch + mouse; tap anywhere)
            function section:AddSlider(cfgs)
                cfgs = cfgs or {}
                local min = tonumber(cfgs.min) or 0
                local max = tonumber(cfgs.max) or 100
                local step = tonumber(cfgs.step) or 1
                local val = math.clamp(tonumber(cfgs.default) or min, min, max)
                local rounding = tonumber(cfgs.rounding) or 0
                local suffix = tostring(cfgs.suffix or "")

                local holder,row = EclipseUI:_makeControlRow(group,44,cfgs.fullWidth)
                local top = create("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,18),Parent=row})
                create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-110,1,0),Text=tostring(cfgs.text or "Slider"),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=top})
                ...
                makeRounded(fill,6)
                attachTooltip(row, cfgs.tooltip)
                ...
                local function set(v, silent)
                    ...
                    val = v
                    local a = (val - min)/math.max(0.0001,(max-min))
                    fill.Size = UDim2.new(a,0,1,0)
                    local t = rounding>0 and (math.floor(v*10^rounding+0.5)/(10^rounding)) or v
                    t = tostring(t)..(suffix~="" and (" "..suffix) or "")
                    valLbl.Text = t
                    if not silent and type(cfgs.callback)=="function" then task.spawn(cfgs.callback, val) end
                end
                set(val, true)

                local function updateFromX(screenX)
                    local rel = math.clamp((screenX - bar.AbsolutePosition.X) / math.max(1, bar.AbsoluteSize.X), 0, 1)
                    set(min + (max-min)*rel)
                end
                ...
                -- Start from bar or fill tap
                for _,hit in ipairs({bar, fill}) do
                    hit.InputBegan:Connect(function(input)
                        if isPointer(input) then beginDrag(input) end
                    end)
                end
            ...
            -- Keybind
            function section:AddKeybind(cfgk)
                cfgk = cfgk or {}
                local holder,row = EclipseUI:_makeControlRow(group,34,cfgk.fullWidth)
                create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-120,1,0),Text=tostring(cfgk.text or "UI Toggle Key"),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
                local btn = create("TextButton",{Size=UDim2.new(0,120,0,26),AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-6,0.5,0),BackgroundColor3=theme.buttonBase,BorderSizePixel=0,Text=keycodeToString(cfgk.default or Enum.KeyCode.RightShift),TextColor3=theme.text,Font=Enum.Font.GothamBold,TextSize=14,Parent=row})
                makeRounded(btn,8); create("UIStroke",{Parent=btn,Color=theme.stroke,Transparency=0.2})
                attachTooltip(row, cfgk.tooltip)
                local waiting=false
                btn.MouseButton1Click:Connect(function()
                    if waiting then return end
                    waiting=true; btn.Text="Press a key..."
                    local conn; conn = UIS.InputBegan:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.Keyboard then
                            ...
                            if type(cfgk.callback)=="function" then task.spawn(cfgk.callback, input.KeyCode) end
                        end
                    end)
                end)
            ...
            -- Input
            function section:AddInput(cfgi)
                cfgi = cfgi or {}
                local holder,row = EclipseUI:_makeControlRow(group,36,cfgi.fullWidth)
                local label = create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-180,1,0),Text=tostring(cfgi.text or "Input"),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
                local box = create("TextBox",{
                    Size=UDim2.new(0,170,1,-10),AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-8,0.5,0),
                    BackgroundColor3=theme.buttonBase,BorderSizePixel=0,Text=tostring(cfgi.default or ""),PlaceholderText=tostring(cfgi.placeholder or ""),
                    TextColor3=theme.text,PlaceholderColor3=theme.subtext,ClearTextOnFocus=(cfgi.clearOnFocus==true),
                ...
                local function commit() if type(cfgi.callback)=="function" then task.spawn(cfgi.callback, box.Text) end end
                box.FocusLost:Connect(function(enter) if enter or not UIS:GetFocusedTextBox() then commit() end end)
            ...
            function section:AddDropdown(cfgd)
                ...
                local lbl = create("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-(requestedValueWidth+12),1,0),Text=tostring(cfgd.text or "Dropdown"),TextColor3=theme.text,Font=Enum.Font.Gotham,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=header,ZIndex=22})
                ...
                valLbl:GetPropertyChangedSignal("Text"):Connect(function() if cfgd.autoValueWidth then layout() end end)
                task.defer(layout)
                ...
                local function close()
                    tweenSizeSafe(listFrame, UDim2.new(1,bleed*2,0,0)); tweenSizeSafe(holder,UDim2.new(1,0,0,collapsedH))
                    valLbl.Text = value .. (open and arrowOpen or arrowClosed)
                    open=false
                end
                local function openNow()
                    ...
                    tweenSizeSafe(listFrame, UDim2.new(1,bleed*2,0,h)); tweenSizeSafe(holder,UDim2.new(1,0,0,collapsedH+h+4))
                    valLbl.Text = value .. arrowOpen; open=true
                end
                ...
                local function rebuild()
                    ...
                    for _,opt in ipairs(options) do
                        ...
                        it.MouseButton1Down:Connect(function()
                            ...
                            valLbl.Text = value .. (open and arrowOpen or arrowClosed)
                            close()
                            if type(cfgd.callback)=="function" then task.spawn(cfgd.callback, value) end
                        end)
                ...
                return {
                    Set=function(_,v) value=tostring(v); valLbl.Text=value..(open and arrowOpen or arrowClosed); layout(); if type(cfgd.callback)=="function" then task.spawn(cfgd.callback,value) end end,
                    Get=function() return value end,
                    SetOptions=function(_,newOpts) options=table.clone(newOpts or options); rebuild(); if open then openNow() end end
                }
            end
    ...
    function window:SetNotifyDock(opts) for k,v in pairs(opts or {}) do self._dockCfg[k]=v end end
    function window:GetNotifyDock() return self._dockCfg end
    function window:SetAccentColor(c) theme.accent=c; accent_pub(c); for _,d in ipairs(gui:GetDescendants()) do if d:IsA("UIStroke") and d.Parent and d.Parent.Name=="ReopenHint" then d.Color=c end end end
    ...
    General:AddKeybind({
        text="UI Toggle Key", default=uiToggleKey, fullWidth=true, tooltip="Key used to show/hide the UI",
        callback=function(kc) uiToggleKey=kc; updateHintText() end
    })

    Perf:AddSlider({
        text="FPS Cap", min=10, max=240, step=5, default=60, rounding=0, suffix="fps", fullWidth=true,
        tooltip="Clamp client FPS (requires executor with FPS API)",
        callback=function(v) local ok=applyFpsCap(v); if not ok then window:Notify("No supported FPS cap API found.",3) end end
    })
    ...
    Noti:AddDropdown({
        text="Dock position", options={"Top Right","Top Left","Left of Window","Right of Window"},
        default=mode2label[dockCfg.mode or "window-left"], fullWidth=true, autoValueWidth=true,
        tooltip="Where notifications appear",
        callback=function(val) window:SetNotifyDock({ mode = label2mode[val] or "window-left" }) end
    })
    Noti:AddSlider({
        text="Vertical offset", min=-40, max=60, step=1, default=dockCfg.offsetY or 8, rounding=0, suffix="px", fullWidth=true,
        tooltip="Shift notifications up/down", callback=function(v) window:SetNotifyDock({ offsetY=v }) end
    })
    ...
    create("TextLabel",{BackgroundTransparency=1,Text="Accent applies to toggles, sliders and notifications.",TextColor3=theme.subtext,Font=Enum.Font.Gotham,TextSize=13,TextWrapped=true,Size=UDim2.new(1,-8,0,0),AutomaticSize=Enum.AutomaticSize.Y,Parent=ThemeSec.Container})
